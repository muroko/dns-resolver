...
const defaultRpc = 'https://eth-rpc-api.dnerochain.xyz/rpc'
...
const getAddressRecord = async(domain, provider) => {
    try {
        const label = namehash(domain + '.dnero')
...
export default class DNS {
    ...
    async getAddress(domain) {
        try {
            const formattedDomain = domain.toLowerCase().replace('.dnero', '')
